 ...
 * Source derived from the public domain JS MD5 implementation by Paul Johnston.
 */
export function md5(message: string): string {
  ...
  const utf8Encode = (str: string) =>
    unescape(encodeURIComponent(str)); // eslint-disable-line deprecated
  ...
  const ff = (
    ...
    aa = addUnsigned(
      aa,
      addUnsigned(addUnsigned((bb & cc) | (~bb & dd), xValue), ac)
    );
  ...
  const gg = (
    ...
    aa = addUnsigned(
      aa,
      addUnsigned(addUnsigned((bb & dd) | (cc & ~dd), xValue), ac)
    );
    return addUnsigned(rotateLeft(aa, s), bb);
  };
  ...
  const hh = (
    ...
    aa = addUnsigned(
      aa,
      addUnsigned(addUnsigned(bb ^ cc ^ dd, xValue), ac)
    );
    return addUnsigned(rotateLeft(aa, s), bb);
  };
  ...
  const ii = (
    ...
    aa = addUnsigned(
      aa,
      addUnsigned(addUnsigned(cc ^ (bb | ~dd), xValue), ac)
    );
    return addUnsigned(rotateLeft(aa, s), bb);
  };
